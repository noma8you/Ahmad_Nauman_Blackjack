...
int bust_score(vector<string> &player1, int player1_bs, map<string, int> &card_value){
    if(player1[player1.size()-1]=="A"){ //when players gets an ace
        int ace_value; //ace's value
        cout<<"Pick Ace value (1 or 11): ";
        cin>>ace_value;
        while(ace_value!=1&&ace_value!=11){
            cout<<"Pick Ace value (1 or 11): "<<endl;
            cin>>ace_value;
        }
        player1_bs=ace_value+player1_bs;
    }
    else{
        player1_bs+=card_value[player1[player1.size()-1]]; //adds value of the latest card
    }
    return player1_bs;
}
...
void determine_winner(int players_bs[], int players_in_game){
    ...
    for(int i=0;i<players_in_game;i++){
        if(players_bs[i]==best_score){
            winners.push_back(i+1); //+1 because players numbered starting from 1
        }
    }
    if(ai_bs==best_score){
    ...
    if(winners.size()==0){
        ...
        cout<<"Tie between ";
        for(int i=0;i<winners.size();i++){ //the players that won
            if(winners[i]==0){
                cout<<"Dealer"; //for more than one winner with dealer
            }
            else{
